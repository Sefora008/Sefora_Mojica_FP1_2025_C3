...
con mayor promedio de lluvias anual. */
...
void Mayor(float R1, float R2, float R3)
{
    if (R1 > R2)
        if (R1 > R3)
        printf("\nRegión con mayor promedoi: Región Golfo. Promedio: &6.2f", R1 / 12);
    else
      printf("\nRegión con mayor promedio: Región Caribe. Promedio: &6.2f", R3 / 12);
    else
        if ( R2 > R3)
        printf("\nRegión con mayor promedio: Región Pacífico. Promedio %6.2f", R3 / 12);
}
